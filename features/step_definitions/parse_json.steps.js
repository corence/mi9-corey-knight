...
module.exports = function() {
	...
	this.When(/^a document is passed with no drm and with episodes available$/, function (callback) {
		try {
			sample_show.drm = false;
			sample_show.episodeCount = 1;
			this.result = ShowParser.extract_available_shows(doc);
		} catch(err) {
			this.exception = err;
		}
		callback();
	...
	this.When(/^a document is passed with drm and with no episodes available$/, function (callback) {
		try {
			sample_show.drm = true;
			sample_show.episodeCount = 0;
			this.result = ShowParser.extract_available_shows(doc);
		} catch(err) {
			this.exception = err;
		}
	...
	this.Then(/^return a populated response list$/, function (callback) {
		...
		this.result.response[0].image.should.be.a('string');
		this.result.response[0].slug.should.be.a('string');
		this.result.response[0].title.should.be.a('string');
		Object.keys(this.result.response[0]).length.should.equal(3);
		callback();
	});

	this.When(/^a non\-json document is passed$/, function (callback) {
		try {
			this.parsed = ShowParser.parse_shows("{something{");
		} catch(err) {
			this.exception = err;
		}
		callback();
	...
	this.Then(/^an exception should be raised$/, function (callback) {
		this.should.have.property('exception');
		callback();
	});
	...
	this.When(/^a document is passed with many shows in it$/, function (callback) {
		...
		this.parsed = ShowParser.parse_shows(JSON.stringify(request));
	...
	this.Then(/^return a populated response list with many shows in it$/, function (callback) {
		...
		"response": [
			...
			this.result.should.deep.equal(expected_result);
		} catch(err) {
			this.exception = err;
		}
